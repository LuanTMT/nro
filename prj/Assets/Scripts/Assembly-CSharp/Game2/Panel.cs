...
namespace Game2
{
    public class Panel : IActionListener, IChatable
    {
        ...
        private void paintEffectItem(mGraphics g, Item item, int x, int y)
        {
            try
            {
                ...
                {
                    if (itemOption2 != null && (itemOption2.optionTemplate.id == 102 || itemOption2.optionTemplate.id == 72 || (itemOption2.optionTemplate.id >= 42 && itemOption2.optionTemplate.id <= 46)))
                    {
	...
	if (isInven)
	{
					...
					if (newSelected == -1)
					{
						int gridStartY_local = bodyRows * (bodyCellH + 2);
					...
					else if (newSelected >= Char.myCharz().arrItemBody.Length)
					{
						...
						int bagStartY = yScroll + yHTN + ((arrBody.Length) / bodyCols) * (bodyCellH) - 10;
        ...
        public void setTabInventory(bool resetSelect)
        {
            ...
            else
            {
                try
                {
                    ...
                    int b = arrItemBag.Length / 5;
                    cmyLim = (arrItemBody.Length + Mathf.Max(5, b)) * ITEM_HEIGHT - hScroll;
                    cmy = (cmtoY = cmyLast[currentTabIndex]);
                    if (cmyLim < 0)
                    {
                        cmyLim = 0;
        ...
        private void paintPetSkill(mGraphics g, bool isPet2)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                if (i == 0)
                {
                    ...
                    mFont.tahoma_7_green2.drawString(g, NinjaUtil.getMoneys(@char.cHPGoc + 1000) + " " + mResources.potential + ": " + mResources.increase + " " + @char.hpFrom1000TiemNang, num2 + 5, num3 + 15, 0);
                }
                if (i == 1)
                {
                    ...
                    mFont.tahoma_7_green2.drawString(g, NinjaUtil.getMoneys(@char.cMPGoc + 1000) + " " + mResources.potential + ": " + mResources.increase + " " + @char.mpFrom1000TiemNang, num2 + 5, num3 + 15, 0);
                }
                if (i == 2)
                {
                    ...
                    mFont.tahoma_7_green2.drawString(g, NinjaUtil.getMoneys(@char.cDamGoc * 100) + " " + mResources.potential + ": " + mResources.increase + " " + @char.damFrom1000TiemNang, num2 + 5, num3 + 15, 0);
                }
                if (i == 3)
                {
                    SmallImage.drawSmallImage(g, 721, num5 + 4, num6 + 4, 0, 0);
                    string st4 = mResources.armor + " " + mResources.root + ": " + NinjaUtil.getMoneys(@char.cDefull);
                    mFont.tahoma_7b_blue.drawString(g, st4, num2 + 5, num3 + 3, 0);
                    mFont.tahoma_7_green2.drawString(g, "Giáp không thể nâng", num2 + 5, num3 + 15, 0);
                }
                if (i == 4)
                {
                    SmallImage.drawSmallImage(g, 719, num5 + 4, num6 + 4, 0, 0);
                    string st5 = mResources.critical + " " + mResources.root + ": " + @char.cCriticalFull + "%";
                    ...
                    mFont.tahoma_7_green2.drawString(g, "Chí mạng không thể nâng", num2 + 5, num3 + 15, 0);
                }
                if (i >= 5)
                {
...
for (int i = 0; i < arrItemBody.Length; i++)
{
    ...
    g.setColor(isSelected ? 16318283 : 9993045);
    g.fillRect(x, y, bodyCellW, bodyCellH - 1);
    g.setColor(16777215, 0.5f);
    g.drawRect(x - 2, y - 2, bodyCellW + 3, bodyCellH + 2);
    if (item != null)
    {
...
int bagCellH = 21;
int bagStartX = xScroll + 2;
int bagStartY = (yScroll + yHTN) + ((arrItemBody.Length) / bodyCols) * (bodyCellH) - 10;
for (int i = 0; i < arrItemBag.Length; i++)
{
    ...
    g.setColor(isSelected ? 16318283 : 11837316);
